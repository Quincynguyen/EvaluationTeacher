...
@section('title', 'Thống kê số lượng đánh giá giảng viên')
@section('content')
...
function drawChart(result) {
     console.log(result);
     var input = [["Point", "Points", { role: "style" }]
     ];

     for (var i = 0; i<result.length ; i++) {
       var item = result[i];
       input[i+1] = [item['subject_name'], parseFloat(item['total'])*100, "#e74a3b"]
     }
     ...
     var options = {  title: "Biểu đồ thống kê số lượng đánh giá dành cho giảng viên",
                      width: 1250,
                      height: 500,
                      bar: {groupWidth: "30%"},
...
<label for="cars">Danh sách Khoa: </label>
<select id="sel" >
